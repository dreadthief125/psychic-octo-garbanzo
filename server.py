...
def openChatWindow():
    ...
    name.focus()
    connectserver=Button(window,text="Connect to the Music Messenger Server",bd=1,font=("Calibri",10),command=connectToServer)
    connectserver.place(x=350,y=6)

    separator=ttk.Separator(window,orient='horizontal')
    separator.place(x=0,y=35,relwidth=1,height=0.1)
    
    labelusers=Label(window,text="Active Sharers",font=("Calibri",10))
    ...
    disconnectButton=Button(window,text="Connect",bd=1,font=("Calibri",10))
    ...
    attach=Button(window,text="Attach & Send Music File",bd=1,font=("Calibri",10))
    ...
    send=Button(window,text="Send Music",bd=1,font=("Calibri",10))
